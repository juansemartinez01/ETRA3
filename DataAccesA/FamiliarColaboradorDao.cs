    ...
    public class FamiliarColaboradorDao:ConnectionToSql
    {
        ...
        public DataTable obtenerFamiliares(int legajoColaborador)
        {
            ...
            using (var connection = GetConnection())
            {
                ...
                {
                    command.Connection = connection;
                    command.CommandText = "SELECT FC.idFamiliar AS 'idFamiliar' ,C.legajo,FC.idDireccion AS 'idDireccion',FC.nombreFamiliar AS 'Nombre',FC.apellidoFamiliar AS 'Apellido',E.nombre AS 'Escolarización',FC.dni AS 'DNI',FC.fechaNacimiento AS 'Fecha Nacimiento',C.legajo, TF.nombre as 'Tipo Familiar', D.nombreCalle, d.numeroCalle,d.piso,d.departamento,d.localidad,d.provincia,CASE WHEN FC.esTrabajador = 0 THEN 'No' WHEN FC.esTrabajador = 1 then 'Si'end as trabaja,CASE WHEN FC.aportes = 0 THEN 'No' WHEN FC.aportes = 1 then 'Si'end as aportes,FC.obraSocial FROM FamiliarColaborador FC JOIN Colaborador C ON FC.legajoColaborador = C.legajo JOIN TipoFamiliar TF ON TF.idTipoFamiliar = FC.tipoFamiliar JOIN Direccion D ON D.id_direccion = FC.idDireccion JOIN Escolaridad E ON E.id = FC.escolaridad WHERE FC.borradoLogico = 0 " + parametros;
